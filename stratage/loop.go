...
func (p *Stratage) BuyAndSell(price, amount string) {
	var (
		order *model.Order
		err   error
	)
...
func (p *Stratage) SellAndBuy(price, amount string) {
	var (
		order *model.Order
		err   error
	)
...
func (p *Stratage) MakeUpBalance() error {
	var (
		err        error
		baseTotal  float64
		quoteTotal float64
